...
class ImageBrightnessFilterTest(DataJuicerTestCaseBase):
    ...
    def test_compute_stats(self):
        ...
        # Check that all three brightness metrics are present
        self.assertIn('brightness_perc_5', scores[0])
        self.assertIn('brightness_perc_99', scores[0])
        self.assertIn('brightness_mean', scores[0])
        # Brightness values should be between 0 and 1
        self.assertGreaterEqual(scores[0]['brightness_perc_5'], 0)
        self.assertLessEqual(scores[0]['brightness_perc_99'], 1)
        self.assertGreaterEqual(scores[0]['brightness_mean'], 0)
        self.assertLessEqual(scores[0]['brightness_mean'], 1)
    ...
    def test_min_brightness(self):
        ...
        perc99_scores = [
            sample[Fields.stats][StatsKeys.image_brightness_scores][0]['brightness_perc_99']
    ...
    def test_max_brightness(self):
        ...
        dataset_with_stats = dataset.map(op.compute_stats)
        perc5_scores = [
            sample[Fields.stats][StatsKeys.image_brightness_scores][0]['brightness_perc_5']
            for sample in dataset_with_stats
        ]
